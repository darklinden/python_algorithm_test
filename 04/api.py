...
class Api(Solution):
    ...
    def algorithm_func(self, nums1: list, nums2: list) -> float:
        ...
        total.sort()
        if len(total) % 2 == 0:
            left = total[int(len(total) / 2 - 1)]
            right = total[int(len(total) / 2)]

            if left in nums1:
                print('left nums1 index: ' + str(nums1.index(left)))
            if left in nums2:
                print('left nums2 index: ' + str(nums2.index(left)))
            if right in nums1:
                print('right nums1 index: ' + str(nums1.index(right)))
            if right in nums2:
                print('right nums2 index: ' + str(nums2.index(left)))

            ret = (left + right) / 2
        else:
            ret = total[int((len(total) - 1) / 2)]
            if ret in nums1:
                print('ret nums1 index: ' + str(nums1.index(ret)))
            if ret in nums2:
                print('ret nums2 index: ' + str(nums2.index(ret)))
        return ret
